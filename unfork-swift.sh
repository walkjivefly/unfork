...
COIN="swiftcash"
DATADIR="$HOME/.swiftcash"
CONFIG="swiftcash.conf"
#
EXPLORER="https://explorer.swiftcash.cc/api/"
#
...
get_explorer_hash() {
    CHAINHASH=`curl --silent "${EXPLORER}block-height/$*" | jq .hash 2>/dev/null`
...
CHAINHIGH=`curl --silent ${EXPLORER}blockcount 2>/dev/null`
