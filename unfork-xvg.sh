...
get_explorer_hash() {
    CHAINHASH=`curl --silent "${EXPLORER}blockhash/$*" | jq .data.blockhash 2>/dev/null`
...
CHAIN=`curl --silent ${EXPLORER}blockcount 2>/dev/null`
CHAINHIGH=$(echo $CHAIN | jq .data.blockcount)
